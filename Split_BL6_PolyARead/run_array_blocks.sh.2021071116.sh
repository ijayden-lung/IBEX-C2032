...
if [ "$task" == "Prepare" ]; then
	...
	python select_positive_from_transcriptome.py --pas_file=$PAS --scan_file=$scanTranscriptome --polyASeqRCThreshold=$polyASeqRCThreshold --RNASeqRCThreshold=$RNASeqRCThreshold --usageThreshold=$usageThreshold --max_shift=$Shift --species=$spe --output=$pos_file
	python select_negative_from_transcriptome.py --pas_file=$pos_file --scan_file=$scanTranscriptome --RNASeqRCThreshold=$RNASeqRCThreshold --max_shift=$Shift --species=$spe --output=$neg_file --prob=$prob
fi
...
if [ "$task" == "get" ]; then
	...
	echo $scan
	echo $bidirection
	python change_all.py  --scan_file=$scan --pre_file=$bidirection  --output=$out --spe=$spe
fi
...
if [ "$task" == "Evaluation" ]; then
	...
	if [ $setName = 'train' ];then
		...
		python change_negative.py --pos_file=$pos_file --neg_file=$negative --scan_file=$scanTranscriptome --pre_file=$predict_res --RNASeqRCThreshold=$RNASeqRCThreshold --max_shift=$Shift --output=$out
	else
		echo
