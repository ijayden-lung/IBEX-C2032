...
spe='hg38'
Shift=12
bestEpoch=0003 #shift12oldpas 311 53 7 162 3#shift12 311 72 21 19 31 #shift8 342 39 51 5  32 # 0.01 178 114 29 49  19 OLDPAS:363 98 57 55 9# THLE2 363 77 13 78 198
#hepg2 236 71  5 56 15#k562_6 139  175 35 44 5#bl6 149 69 66 15 4#brain 258 25 12 168 1#thle2 186 37 17 102 12#huh7 182 49 19 41 21#snu398 341 271 91 334 206 #k562 138 111 47 33 10 #hegp2 236 52 105 12 143
round=4
learning_rate=6e-4 #####small learning rate for fine tuning
task='statistics'   ##tak = [preparation training evaluation statistics]
...
if [ "$cell" == "hepg2" ]; then
	...
	penality=1
	trainid="${sample}.pAs.single_kermax6"
	dir="../Split_BL6/K562_Control_data"
	DB="../Split_BL6_PolyARead/usage_data/K562_Control.pAs.merge.coverage.txt"
	info="../Split_BL6/K562_Control_data/info.txt"
...
if [ "$task" == "preparation" ]; then
	...
	echo "starting task $task"
	python merge_postpropose2.py --root_dir=$dir --round=$roundName --trainid=$Testid --sets='train' --maxPoint $maxPoint --penality $penality
	python merge_postpropose2.py --root_dir=$dir --round=$roundName --trainid=$Testid --sets='valid' --maxPoint $maxPoint --penality $penality
    python merge_postpropose2.py --root_dir=$dir --round=$roundName --trainid=$Testid --sets='test' --maxPoint $maxPoint --penality $penality
	python merge_postpropose2.py --root_dir=$dir --round=$roundName --trainid=$Testid --sets='all' --maxPoint $maxPoint --penality $penality
