...
my $chr = "chr6";
my $strand = "+";
my $start = 57053243-300;
my $end   = 57059044+300;
my $symbol = "KIAA1586";
my $block_dir = "../../Split_BL6/THLE2_Control_data";
my $block = &find_block($start,$end,$chr,$strand,$block_dir);
my $score_file = "../predict/THLE2_Control.pAs.single_kermax6.THLE2_Control_aug12_SC_p1r0.05u0.05_4-0029.$chr\_$strand\_$block.txt";
my $coverage_file = "$block_dir/$chr\_$strand\_$block";
my $polya_file = "awk '(\$1==\"$chr\" && \$6==\"$strand\")' ../../Split_BL6/polyA_seq/stackTHLE2.bed |";
my $pos_file = "../Figures/thle2/predicted.txt";
my $times = 8.9;
open OUT,">../Figures/THLE2/THLE2.$symbol.bedGraph";
...
my $score_track = "track type=bedGraph name=\"THLE2 Prediction Score\" visibility=full color=200,100,0 altColor=0,100,200 priority=20";
&write_bedGraph($score_ref,$score_track);
my $coverage_ref  = &read_file($coverage_file);
my $coverage_track = "track type=bedGraph name=\"THLE2 RNA-Seq RPM\" visibility=full color=255,0,0 altColor=0,100,200 priority=20";
&write_bedGraph($coverage_ref,$coverage_track);
my $polya_ref  = &read_polya_file($polya_file,$chr,$strand,$times);
my $polya_track = "track type=bedGraph name=\"THLE2 PolyA-Seq RPM\" visibility=full color=0,204,0 altColor=0,100,200 priority=20";
&write_bedGraph($polya_ref,$polya_track);
my $position_ref = &read_position_file($pos_file,$chr,$strand);
my $position_track = "track type=bedGraph name=\"THLE2 Predicted Position\" visibility=full color=0,204,255 altColor=0,100,200 priority=20 yLineMark=12 yLineOnOff=on";
