...
my $end   = 36453784+300;
my $symbol = "ANLN";
my $block_dir = "../../Split_BL6/K562_Chen_data";
my $block = &find_block($start,$end,$chr,$strand,$block_dir);
my $score_file = "../predict/K562_Chen.pAs.single_kermax6.K562_Chen_aug12_SC_p3.5r0.05u0.05_4-0018.$chr\_$strand\_$block.txt";
my $coverage_file = "$block_dir/$chr\_$strand\_$block";
my $polya_file = "awk '(\$1==\"$chr\" && \$6==\"$strand\")' ../../Split_BL6/polyA_seq/stackK562.bed |";
my $pos_file = "../Figures/K562/predicted.txt";
my $times = 33.3;
open OUT,">../Figures/K562/K562.$symbol.bedGraph";
...
my $score_track = "track type=bedGraph name=\"K562 Prediction Score\" visibility=full color=200,100,0 altColor=0,100,200 priority=20";
&write_bedGraph($score_ref,$score_track);
my $coverage_ref  = &read_file($coverage_file);
my $coverage_track = "track type=bedGraph name=\"K562 RNA-Seq RPM\" visibility=full color=255,0,0 altColor=0,100,200 priority=20";
&write_bedGraph($coverage_ref,$coverage_track);
my $polya_ref  = &read_polya_file($polya_file,$chr,$strand,$times);
my $polya_track = "track type=bedGraph name=\"K562 PolyA-Seq RPM\" visibility=full color=0,204,0 altColor=0,100,200 priority=20";
&write_bedGraph($polya_ref,$polya_track);
my $position_ref = &read_position_file($pos_file,$chr,$strand);
my $position_track = "track type=bedGraph name=\"K562 Predicted Position\" visibility=full color=0,204,255 altColor=0,100,200 priority=20 yLineMark=12 yLineOnOff=on";
