...
my $end   = 36453784+300;
my $symbol = "ANLN";
my $block_dir = "../../Split_BL6/HepG2_Control_data";
my $block = &find_block($start,$end,$chr,$strand,$block_dir);
my $score_file = "../predict/HepG2_Control.pAs.single_kermax6.HepG2_Control_aug12_SC_p1r0.05u0.05_4-0010.$chr\_$strand\_$block.txt";
my $coverage_file = "$block_dir/$chr\_$strand\_$block";
my $polya_file = "awk '(\$1==\"$chr\" && \$6==\"$strand\")' ../../Split_BL6/polyA_seq/stackHepG2.bed |";
my $pos_file = "../Figures/HepG2/predicted.txt";
my $times = 6.2;
open OUT,">../Figures/HepG2/HepG2.$symbol.bedGraph";
...
my $score_track = "track type=bedGraph name=\"HepG2 Prediction Score\" visibility=full color=200,100,0 altColor=0,100,200 priority=20";
&write_bedGraph($score_ref,$score_track);
my $coverage_ref  = &read_file($coverage_file);
my $coverage_track = "track type=bedGraph name=\"HepG2 RNA-Seq RPM\" visibility=full color=255,0,0 altColor=0,100,200 priority=20";
&write_bedGraph($coverage_ref,$coverage_track);
my $polya_ref  = &read_polya_file($polya_file,$chr,$strand,$times);
my $polya_track = "track type=bedGraph name=\"HepG2 PolyA-Seq RPM\" visibility=full color=0,204,0 altColor=0,100,200 priority=20";
&write_bedGraph($polya_ref,$polya_track);
my $position_ref = &read_position_file($pos_file,$chr,$strand);
my $position_track = "track type=bedGraph name=\"HepG2 Predicted Position\" visibility=full color=0,204,255 altColor=0,100,200 priority=20 yLineMark=12 yLineOnOff=on";
...
sub read_file{
	...
	while(<FILE>){
		...
		my ($chromosome,$pos,$srd) = split /\:/,$pas_id;
		if($chromosome eq $chr && $srd eq $strand && $start<=$pos && $end >= $pos){
			$hash{$pos} = $score;
		}
	}
	return \%hash;
