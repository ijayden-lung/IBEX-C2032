...
spe='K562'
bestEpoch=0269  #201 22  HepG2 175 162 195 75
round=0
...
if [ "$task" == "preparation" ]; then
	...
	sbatch --job-name='Prepare' --array=0-$block_idx --export=dir=$dir,ENS=$ENS,PAS=$PAS,round=$roundName,task='Prepare',augmentation=$augmentation,prob=$prob,polyASeqRCThreshold=$polyASeqRCThreshold,RNASeqRCThreshold=$RNASeqRCThreshold,usageThreshold=$usageThreshold,Shift=$Shift run_array_blocks.sh
