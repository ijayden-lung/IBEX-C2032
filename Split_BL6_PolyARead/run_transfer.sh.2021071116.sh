...
old='k562'
cell='thle2' #[k562,hepg2,huh7,thle2,snu398]
spe='hg38'
round=4
learning_rate=5e-4 #####small learning rate for fine tuning
task='get'   ##task = [preparation training evaluation statistics]
transfer="Finetune" ##[Transfer Finetune]
bestEpoch=0002   #thle2Tohepg2 69 k562Tothle2 2 thle2Tothle2 2
trainid="${transfer}_${old}Tothle2"
...
if [ "$task" == "training" ]; then
	...
	echo "starting task $task"
	sbatch --job-name='get' --array=0-$block_idx --export=dir=$dir,ENS=$ENS,PAS=$PAS,Testid=$Testid,maxPoint=$maxPoint,penality=$penality,task='get',spe=$spe run_array_blocks.sh
