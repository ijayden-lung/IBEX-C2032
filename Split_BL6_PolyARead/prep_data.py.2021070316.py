...
def get_files(root_dir,block_dir,RoundName,label):
	...
	test_index = np.random.choice(range(len(files)),len(files)%Fold,replace=False)
	####The remaining sets as train and valid sets
	new_files = np.delete(files,test_index)
	#### 5 Fold cross validation
	kf = KFold(n_splits = Fold,random_state=2021,shuffle=True)
	train_round = []
	valid_round = []
...
def get_data(root_dir,block_dir,polyA_file,RoundName,label,subset,length):
	"""
	...
	for file_path in sets:
		with open(file_path,'r') as f:
			for line in f:
				...
				if line[0] !='#pas_id' and line[0] !='pas_id' and (subset == 'train' or (subset=='valid' and 'Origin' in line[5])):
					pas_id = line[0]
					...
					COVERAGE = np.array(line[8:8+length]).astype(np.float32)
...
def prep_data(ROOT_DIR,BLOCK_DIR,polyA_file,ROUNDNAME,NUM_FOLDS,subset,length,OUT=None):
	pos_data1, pos_data2, pos_data3, pos_meta_data,pos_labels = get_data(ROOT_DIR,BLOCK_DIR,polyA_file,ROUNDNAME, 'positive',subset,length)
	...
	neg_data1, neg_data2, neg_data3, neg_meta_data,neg_labels = get_data(ROOT_DIR,BLOCK_DIR,polyA_file,ROUNDNAME, 'negative',subset,length)
...
class DataGenerator(Sequence):
	...
	def __data_generation(self, list_IDs_temp):
		...
		X = {"seq_input": seq_data, "cov_input": cov_data}
...
if __name__ == "__main__":
	...
	length=opts.window
	############Global  Parameters ############
	prep_data(ROOT_DIR,BLOCK_DIR,polyA_file,ROUNDNAME,NUM_FOLDS,OUT,length)
