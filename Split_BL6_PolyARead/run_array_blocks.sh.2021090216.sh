...
if [ "$task" == "get" ]; then
	...
	#bidirection="map/${Testid}.${baseName}.peak.${maxPoint}.${penality}.txt"
...
if [ "$task" == "Evaluation" ]; then
	...
	#New="Yes"
	#python3 scanTranscriptome.py --testid $Testid --baseName $baseName --threshold $maxPoint --penality=$penality
	#python3 statScan.py --testid $Testid --baseName $baseName --threshold $maxPoint --penality=$penality --info $info --pasfile $PAS --polyASeqRCThreshold $polyASeqRCThreshold --RNASeqRCThreshold $RNASeqRCThreshold --usageThreshold $usageThreshold
	#predict_res="map/${Testid}.${baseName}.peak.${maxPoint}.${penality}.txt"
