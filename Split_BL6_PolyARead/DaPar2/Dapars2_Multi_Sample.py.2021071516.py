...
def parse_cfgfile(cfg_file):
    ...
    parser.add_argument('--wig_file', help='wiggle file')
    parser.add_argument('--out_dir', help='output directory')
    parser.add_argument('--utr_file', help='utr file')
    parser.add_argument('--res_file', help='output file basename')
    parser.add_argument('--depth_file', help='seq depth file')
    parser.add_argument('--chr', help='chromosome')

    args = parser.parse_args()
    Aligned_Wig_files=args.wig_file.split(',')
    output_directory=args.out_dir
    Annotated_3UTR_file=args.utr_file
    Output_result_file=args.res_file
    Coverage_threshold = 10
    Num_threads = 1
    sequencing_depth_file = args.depth_file
    curr_processing_chr  = args.chr
    return Aligned_Wig_files, output_directory, Annotated_3UTR_file, Output_result_file, sequencing_depth_file, Num_threads, Coverage_threshold,curr_processing_chr
...
def De_Novo_3UTR_Identification_Loading_Target_Wig_for_TCGA_Multiple_Samples_Multiple_threads_Main3_shared_list(argv=None):
    ...
    if len(sys.argv) == 1:
        ...
        exit(1)
    cfg_file = sys.argv[1]
    #curr_processing_chr = sys.argv[-1]
    #curr_processing_chr = sys.argv[2]
    print("[%s] Start Analysis ..." % time_now())
    Group1_Tophat_aligned_file, output_directory, Annotated_3UTR_file, Output_result_file, sequencing_depth_file, Num_threads, Coverage_threshold,curr_processing_chr = parse_cfgfile(cfg_file)
    ...
    print("All samples Joint Processing %s ..." % curr_processing_chr)
    print("[%s] Loading Coverage ..." % time_now())
    ...
    print("[%s] Loading Coverage Finished ..." % time_now())
...
def Assign_to_different_processor_balance(Total_number, num_processors):
    Assigned_results = []
    num_each_processor = int(Total_number/num_processors)
    ...
    if num_each_processor == 0:
        ...
        for i in range(int(num_processors-remain)):
            Assigned_results.append(range(int(i*num_each_processor+remain*(num_each_processor+1)), int((i+1)*num_each_processor+int(remain)*(num_each_processor+1))))
...
def Assign_to_different_processor_balance_events(All_events_ids, num_processors):
    ...
    if num_each_processor == 0:
        ...
        remain = Total_number - num_processors * num_each_processor
        for i in range(int(remain)):
            Assigned_results.append(range(int((i)*(num_each_processor+1)), int((i+1)*(num_each_processor+1))))

        for i in range(int(num_processors-remain)):
            Assigned_results.append(range(int(i*num_each_processor+remain*(num_each_processor+1)), int((i+1)*num_each_processor+remain*(num_each_processor+1))))
