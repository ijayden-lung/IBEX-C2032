...
if [ "$task" == "Evaluation" ]; then
	...
	if [ $setName = 'train' ];then
		...
		python change_negative.py --pos_file=$pos_file --neg_file=$negative --scan_file=$scanTranscriptome --pre_file=$predict_res --RNASeqRCThreshold=$RNASeqRCThreshold --max_shift=$Shift --output=$out --window=$window --usage_file=$PAS --polyASeqRCThreshold=$polyASeqRCThreshold
	else
		echo
