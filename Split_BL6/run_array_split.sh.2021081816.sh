...
if [ "$spe" == "hg38" ]; then
	...
	GEN="/home/longy/cnda/gencode/gencode.v38.annotation.gtf.gz"
elif [ "$spe" == "mm10" ]; then
...
if [ "$task" == "Coverage" ]; then
	python extract_coverage_from_scanGenome.py --pas_file=$DB --scan_file=$scanTranscriptome --species=$spe
	#python extract_coverage_from_scanGenome.py --pas_file=$ENS --scan_file=$scanTranscriptome --file_type='ensembl' --species=$spe
	python extract_coverage_from_scanGenome.py --pas_file=$GEN --scan_file=$scanTranscriptome --file_type='gencode' --species=$spe
fi
