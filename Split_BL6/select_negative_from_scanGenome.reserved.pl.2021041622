...
&fulfill("ncRNA");
##########When all ncRNA without motif and all of the ground true with motif.
if($negative_pas_type_count{"ncRNA"} < $true_pas_type_count{"ncRNA"}){
	####Allow pAs without motif
	$true_motif_count{0} +=  $true_pas_type_count{"ncRNA"}-$negative_pas_type_count{"ncRNA"};
	####Add the number of negative pAs with motif
	$negative_motif_count{1} += $true_motif_count{0};
	&fulfill("ncRNA");
}
&fulfill("intergenic");
##########When all ncRNA without motif and all of the ground true with motif.
if($negative_pas_type_count{"intergenic"} < $true_pas_type_count{"intergenic"}){
	####Allow pAs without motif
	$true_motif_count{0} +=  $true_pas_type_count{"intergenic"}-$negative_pas_type_count{"intergenic"};
	####Add the number of negative pAs with motif
	$negative_motif_count{1} += $true_motif_count{0};
	&fulfill("intergenic");
}
...
sub fulfill{
	my ($PAS_TYPE) = @_;
	#foreach my $pas_id (keys %negative_candidate){
...
while(my ($key,$true_count) = each %true_pas_type_count){
	...
	if($negative_count<$true_count){
		warn "Warning! At $scanTranscriptome Number of pseudo PAS with pastype $key = $negative_count";
		warn "It is not enough compared to ground truth = $true_count\n";
	}
}
if($total_count<$true_pas_num){
	warn "Warning! At $scanTranscriptome Number of pseudo PAS=$total_count";
	warn "It is not enough compared to ground truth=$true_pas_num\n";
}
else{
	print "Successfully save $total_count pAs with Shift $Shift\n";
}
