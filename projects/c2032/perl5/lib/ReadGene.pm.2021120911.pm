...
sub GetGeneInfo{
	...
	&FirstInfo($ENS);
	&TreatGene();
	my ($gene_location_ref,$gene_start_of_ref,$intergene_length_ref)= &get_intergenic_length($Extend);
	print "$biotype{'MTND2P28'}\n";
	return (\%exon_loc,\%tss,\%utr,\%gene_start,\%gene_end,\%biotype,$intergene_length_ref,$gene_location_ref,$gene_start_of_ref);
}
...
sub FirstInfo{
	...
	while(<ENS>){
		...
		if($type eq "gene"){
			...
			my $gene_name = $gene_info{'gene_name'};
			my $gene_type = $gene_info{'gene_type'};
			$gene_id =~ s/\"|\;//g;
			$gene_name =~ s/\"|\;//g;
			$gene_type =~ s/\"|\;//g;
			...
			my $gene_name = $gene_info{'gene_name'};
			my $trans_type = $gene_info{'transcript_type'};
			my $trans_id  = $gene_info{'transcript_id'};
			$gene_name =~ s/\"|\;//g;
			$trans_type =~ s/\"|\;//g;
			next if ! exists $biotype{$gene_name};
			$exon_loc{$gene_name}->{$trans_id}->{"$start,$end"} = '';
			if($srd eq "+"){
				if(!exists $compare{$gene_name} || $compare{$gene_name} < $end){
					if($trans_type eq "protein_coding" || $biotype{$gene_name} ne "protein_coding"){
						...
						if(!exists $gene_end{"$chr:$srd"}->{$gene_name}){
							$gene_end{"$chr:$srd"}->{$gene_name} = $end;
						}
					}
				}
...
sub get_intergenic_length{
	...
	while(my ($location,$gene) = each %$gene_location_ref){
		...
		for(my $i=0;$i<@$gene-1;$i++){
			...
			if($gene1 eq "MTND2P28"){
				print "$gene2\t$gene2_start\t$intergene_length\n";
			}
	...
	return ($gene_location_ref,$gene_start_of_ref,\%intergene_length);
}
...
sub Sort_gene{
	...
	while(my ($location,$val) = each %gene_end){
		if($location =~ /\+/){
			foreach my $key (sort{$val->{$a} <=> $val->{$b} ||  $gene_start{$location}->{$a}<=>$gene_start{$location}->{$b}} keys %$val){
				push @{$gene_location{$location}},$key;
			}
		}
		elsif($location =~ /\-/){
			foreach my $key (sort{$val->{$b} <=> $val->{$a} ||  $gene_start{$location}->{$b}<=>$gene_start{$location}->{$a}} keys %$val){
				push @{$gene_location{$location}},$key;
			}
		}
	}
	while(my ($location,$val) = each %gene_start){
		if($location =~ /\+/){
			foreach my $key (sort{$val->{$a} <=> $val->{$b} ||  $gene_end{$location}->{$a}<=>$gene_end{$location}->{$b}} keys %$val){
				push @{$gene_start_of{$location}},$key;
			}
		}
		elsif($location =~ /\-/){
			foreach my $key (sort{$val->{$b} <=> $val->{$a} ||  $gene_end{$location}->{$b}<=>$gene_end{$location}->{$a}} keys %$val){
				push @{$gene_start_of{$location}},$key;
			}
		}
	}
