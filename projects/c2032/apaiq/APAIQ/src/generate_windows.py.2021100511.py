...
def split_chr_bedGraph2(root_dir,input_file,strand,window,block_length,fa_file,depth,name):
	chr_dict = dict()
	blocks = dict()
	block = []
	reference = dict()
		...
		for i,line in enumerate(lines):
			...
			if chromosome not in chr_dict.keys():
				...
				for pos in range(pos1-extend1,pos2+extend2):
					...
					#block.append('%s:%s:%s\t%.5f\t%s'%(chromosome,pos,strand,rpm,base))
					count += 1
			else:
...
def Generate_windows(root_dir,input_file,input_plus,input_minus,fa_file,keep_temp,window,name,depth):
	...
	if(input_plus is not None):
		...
		if('wig' in input_plus):
			...
			plus_blocks = split_chr_bedGraph2(root_dir,input_plus,strand,window,block_length,fa_file,depth,name)
		else:
			sys.exit("input file extension should be wig or bedGraph")
		#wig_chr_files = glob.glob(root_dir+"/*"+strand+".wig")
		#for wig_file in wig_chr_files:
	...
	if(input_minus is not None):
		...
		if('wig' in input_minus):
			...
			minus_block = split_chr_bedGraph2(root_dir,input_minus,strand,window,block_length,fa_file,depth,name)
		else:
			sys.exit("input file extension should be wig or bedGraph")
		#wig_chr_files = glob.glob(root_dir+"/*"+strand+".wig")
		#for wig_file in wig_chr_files:
