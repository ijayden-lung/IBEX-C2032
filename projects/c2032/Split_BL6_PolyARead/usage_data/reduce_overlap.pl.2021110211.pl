...
=cut
#=pod
my $COV = "THLE2_Control.pAs.coverage.txt";
my $COV2 = "THLE2_Control.pAs.gen.coverage.txt";
my $usage = "/home/longy/project/Split_BL6/polyA_seq/THLE2_Control.pAs.usage.txt";
#my $out = "THLE2_Control.pAs.old.usage.txt";
#my $usage = "THLE2_Control.pAs.old.usage.txt";
my $out  = "THLE2_Control.pAs.usage.txt";
...
foreach(@USAGE){
	...
	if(exists $dist{$symbol}){
		...
		if($dist_diff <= $dist_threshold){
			...
			$remove = $remove_old; ####Use remove old or new;
			$remove_id{$remove} = '';
		}
	}
	if($pas_id eq "chr1:1335352:-"){
		print "$pas_id\t$readCount\t$readCount{$symbol}\n";
		print "$remove\n";
	}
	if($remove ne $pas_id){
		$dist{$symbol} = $pos;
...
foreach(@USAGE){
	...
	my ($pas_id,$pas_type,$chr,$pos,$strand,$symbol,$usage,$readCount,$motif,$ave_diff) = split;
	#next if ! exists $ConserveOf{$pas_id} && $ave_diff<1;
	my $coverage_ref = $coverage1{$pas_id};
	$data[10]  = &TrimmedMean(@$coverage_ref);
