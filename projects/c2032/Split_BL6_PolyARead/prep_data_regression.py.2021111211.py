	...
	with open(file_path,'r') as f:
		for line in f:
			...
			if (line[0] !='#pas_id' and line[0] !='pas_id'):
				...
				label = float(line[8])/depth
				if(label > 100000 or np.max(COVERAGE)>100000):
					#print('filter too high'+pas_id)
					continue
				if(line[1] == 'False' or label==0):
					#print('filter false'+pas_id)
					continue
...
def prep_data(file_path,NUM_FOLDS,depth,OUT=None):
	data1,labels,pasid = get_data(file_path,depth)
...
class DataGenerator(Sequence):
	...
	def __init__(self, data, labels, pas_id,shift, length=1001,batch_size=32, shuffle=True):
		...
		self.truncate_length = int((data.shape[1]-length)/2)
		self.on_epoch_end()
	...
	def __data_generation(self, list_IDs_temp):
		...
		for i, ID in enumerate(list_IDs_temp):
			...
			if self.shift >0:
				...
				offset = self.truncate_length-self.shift+random
			else:
				offset = self.truncate_length
...
if __name__ == "__main__":
	...
	depth   = opts.depth
	############Global  Parameters ############
	prep_data(FILE_PATH,NUM_FOLDS,deoth,OUT)
