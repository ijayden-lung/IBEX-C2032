...
def CNN(inputs):
	x = Conv1D(filters= 16, kernel_size= 6, padding = 'valid', kernel_regularizer = regularizers.l2(1e-4), bias_regularizer = regularizers.l2(1e-4))(inputs)
...
def evaluate(x,y,pasid,trainid,dataset,label_mean,label_std,length,bestEpoch=2000):
...
if __name__ == "__main__":
	...
	validation_generator = DataGenerator(valid_x,valid_y,valid_pasid,0,length)
	...
	evaluate(train_x,train_y,train_pasid,trainid,'train',label_mean,label_std,length,epochs)
	evaluate(valid_x,valid_y,valid_pasid,trainid,'valid',label_mean,label_std,length,epochs)
