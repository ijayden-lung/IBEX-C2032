...
#perl generate_bed.pl $out

#qapa build --db examples/hg38/ensembl.identifiers.txt -o examples/hg38/${out}.polyA_sites.bed examples/hg38/hg38_genes.genePred >examples/hg38/${out}.output_utrs.bed


#qapa fasta -f ~/cnda/ucsc/hg38.fa examples/hg38/${out}.output_utrs.bed examples/hg38/${out}.output_sequences.fa


#salmon index -t examples/hg38/${out}.output_sequences.fa -i examples/hg38/${out}.utr_library

#salmon quant -i examples/hg38/${out}.utr_library  -l ISR -1 $FQ1 -2 $FQ2 --validateMappings -o $out -p 16


qapa quant --db examples/hg38/ensembl.identifiers.txt $out/quant.sf > $out/pau_results.txt
