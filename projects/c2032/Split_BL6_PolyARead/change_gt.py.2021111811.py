...
def read_predict(pas_file,CHR,SRD,thre):
    f = open(pas_file, 'r')
    pas_dict= dict()
    ...
    for line in f.readlines():
        ...
        if(chromosome == CHR and strand == SRD and float(RNASeqRPM)>=0.05 and float(polyARead)>=thre and float(usage)>=0.05):
            pos = int(pos)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--thre', default=1,type=float, help='window size')
    ...
    pas_dict = dict()
    pas_dict = read_predict(pre_file,chromosome,strand,thre)
