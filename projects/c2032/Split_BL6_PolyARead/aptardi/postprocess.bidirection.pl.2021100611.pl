...
while(<FILE>){
	chomp;
	next if $_ =~ /\#/;
	my ($chr,$source,$feature,$start,$end,$prob,$strand) = split;
	if ($feature eq "transcript" && $source eq 'aptardi'){
		my $pos = $end;
		if($strand eq "-"){
			$pos = $start;
		}
		...
		next if $prob < 1e-3;
		if(!exists $max_pas{"$chr:$pos:$strand"} || $max_pas{"$chr:$pos:$strand"}<$prob){
			$max_pas{"$chr:$pos:$strand"} = $prob;
		}
	}
}

my $total = keys %max_pas;
my %pas_pos;
while (my ($key,$val)  = each %max_pas){
	my ($chr,$pos,$strand) = split /\:/,$key;
	$pas_pos{"$chr:$strand"}->{$pos}  = $val;
}
