...
def parse_args(): 
    ...
    parser.add_argument('--p', '-p', type = check_prob, help = "probability threshold, predictions >= value will be classified as polyA sites (otherwise default used)", default = 0, required = False, const = 'default', nargs = '?', metavar = "value in (0, 1)")
...
def final_gtf(in_preds, gtf_file, fo, thresh, tmp_dir):
    def bed_line(estart_val, eend_val, field_vals, nline_val, in_att, in_lab):
        ...
        return out_line
    def write_gtf_lines(elems, in_id, num_trans,polya_prob):
        fin_list = []                   
        chrom = elems[0]
        att = elems[4]
        source_name = elems[8]
        ...
        count = 0
        fin_list.append([chrom, sources[0], 'transcript', str(start), str(end), polya_prob, strand, '.', fin_attribs[0]])
        for j in range(block_count):
            ...
            count += 1
            fin_list.append([chrom, source_group, 'exon', str(exon_start), str(exon_end), polya_prob, strand, '.', group])
        return fin_list
    tot_line = 0
    estart = []
    eend = []
        ...
        if type_field == 'transcript' or (prevtransid != '' and transid != '' and transid != prevtransid):
            if len(estart) != 0:
                if len([i for i in in_preds if i[1].split('|')[0] == prevtransid and i[1].split('|')[1] == prevgeneid]) == 0:
                    ...
                    all_lines = write_gtf_lines(line, prevtransid, tot_line,'2')
                    ...
                    tot_line += 1
                    line = bed_line(estart, eend, prevfield, 0, ','.join(all_att), ','.join(all_label))
                    all_lines = write_gtf_lines(line, prevtransid, tot_line,'3')
                    for fin_lines in all_lines:
                        fo.writelines('\t'.join(fin_lines) + '\n')
                    for x in curr_preds[0]:
                        ...
                        if prevstrand == '-' and not reg_start+1 <= int(float(estart[0])) <= reg_end and not abs(reg_start+1 - int(float(estart[0]))) <= 100:
                            ...
                            all_lines = write_gtf_lines(line, prevtransid, tot_line,str(polya_prob))
                            for fin_lines in all_lines:
                                fo.writelines('\t'.join(fin_lines) + '\n')
