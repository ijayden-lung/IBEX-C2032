...
open FILE,"awk '(\$4 == \"+\")' $data | sort -k 2,2 -k 3,3n | ";
while(<FILE>){
	chomp;
	next if $_ =~ /^pas_id/;
	my ($pas_id,$chr,$pos,$strand,$is_true,$db_pasid) = split;
	my $sign = 1;
	...
	$pas_type{$pas_id} = "$new_pas_type\t$chr\t$pos\t$strand\t$new_symbol\t$is_true\t$db_pasid";
	$pas_gene{$pas_id}  = $new_symbol;
}

open FILE,"awk '(\$4 == \"-\")' $data | sort -k 2,2 -k 3,3nr | ";
while(<FILE>){
	chomp;
	next if $_ =~ /^pas_id/;
	my ($pas_id,$chr,$pos,$strand,$is_true,$db_pasid) = split;
