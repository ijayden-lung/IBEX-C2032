...
use AddPas;
my $thle_file = "THLE2_Control.usage.txt";
...
#open OUT2,">../Figures/stat.rpm.usage0.05.txt";
...
my @k562_thre = qw/0 3.5 10 17 34 67 100 134 167 200 234 265 300 334/;
...
sub statistics{
	...
	foreach my $i (@thre){
		my ($count,$gt_readcount) =  &count_polyAread($file2,$i,$all_pas_ref);
		$polyA_total{$i} = $count;
		$polyA_readcount{$i} = $gt_readcount;
	}
	my %polyA_correct;
	open FILE,"$file";
	<FILE>;
	...
	while(<FILE>){
		...
		foreach my $i (@thre){
			#if(exists $polyA_readcount{$i}->{$gt_pas} && abs($diff)<25){
			if(exists $polyA_readcount{$i}->{$nearest_pas} && abs($diff)<25){
				$polyA_correct{$i}++;
			}
		}
		#if(!exists $polyA_readcount{1}->{$nearest_pas} && $polyA_readcount>=1){
		#	print "$cell\t$_\n";
		#}
