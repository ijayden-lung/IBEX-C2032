...
my %gene = (#"AP1AR"=>['chr4','+',112269096,112270053],
			...
			#Both
			"PHKB"=>['chr16','+',47699639,47699741],

			);
&cell_line("K562","RNASeq");
#&cell_line("K562","sequence");
#&cell_line("K562","both");
sub cell_line{
	my ($cell,$type) = @_;
	my $rep;
	...
	&main("anno",$cell,$block_dir,$anno_base,$times) if $cell eq "K562";
	...
	if($type eq "both"){
		...
		$type = "sequence";
		my $pos_base3 = "../Figures/$cell/predicted.$type.txt";
		&main("pos","$type Only",$block_dir,$pos_base3,$times);
	}
	else{
		my $pos_base2 = "../Figures/$cell/predicted.$type.txt";
		&main("pos","$type Only",$block_dir,$pos_base2,$times);
