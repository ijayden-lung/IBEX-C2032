...
while(<FILE>){
	...
	next if $_ =~ /^pas_id/;
	my @data = split /\t/;
	my ($pas_id,$true,$chr,$pos,$strand,$symbol,$motif) = @data[0..6];
	if($true eq "False"){
		$false_count++;
	}
	...
	$pas_type{$pas_id} = "$true\t$chr\t$pos\t$strand\t$symbol";
	my @cov = @data[6..$#data];
	$coverage{$pas_id} = \@cov;
...
foreach(@INP){
	...
	print OUT "$pas_id\t$pas_type{$pas_id}\t$biotype\t$usage\t$ave\t",join("\t",@$cov_ref),"\n";
}
