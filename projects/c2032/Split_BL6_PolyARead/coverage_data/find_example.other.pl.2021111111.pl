...
sub Get_Predict_Usage{
	...
	while(<FILE>){
		chomp;
		my ($pas_id,$pas_type,$symbol,$usage,$gt_pasid,$gt_diff,$score) = (split)[0,1,5,6,-5,-4,-1];
		next if $score < 12;
		next if abs($gt_diff) > 25;
		next if $pas_type ne "Intronic";
...
my $thle2_pre_ref = &Get_Predict_Usage("../Figures/SNU398/new_pastype.predicted.SANPolyA.txt");
...
while (my ($key,$val) = each %snu398_pre){
	foreach my $id (keys %$val){
		if((exists $snu398_hash{$key}->{$id} && $snu398_hash{$key}->{$id}>0.3)){
			print "$key\t$id\n";
		}
	}
}
