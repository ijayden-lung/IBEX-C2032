...
my ($exon_loc_ref,$utr_ref,$gene_start_ref,$gene_end_ref,$biotype_ref,$intergene_length_ref,$gene_location_ref,$gene_start_of_ref) = &GetGeneInfo($ENS,$Extend);
...
while(<FILE>){
	...
	next if $_ =~ /^pas_id/;
	my @data = split /\t/;
	my ($pas_id,$pas_type,$chr,$pos,$strand,$symbol) = @data[0..5];
	next if !exists $valid_pas{$pas_id};
	my $sign = 1;
	##count if the start of mapping pas to gene,update every time;
	my ($count,$new_pas_type,$new_symbol,$extend_length,$between_gene) = &Assign_pAs_To_Gene($exon_loc_ref,$count_for_ref->{"$chr:$strand"},$sign,$pos,$Extend_Info,$utr_ref,$gene_start_ref->{"$chr:$strand"},$gene_end_ref->{"$chr:$strand"},$biotype_ref,$gene_start_of_ref->{"$chr:$strand"},$intergene_length_ref);
...
foreach(@INP){
	...
	my $cov_ref = $coverage{$pas_id};
	print OUT "$pas_id\t$pas_type{$pas_id}\t$remark\t$biotype\t$usage\t$ave\t",join("\t",@$cov_ref),"\n";
}
