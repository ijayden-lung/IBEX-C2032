								...
								for i, peak_ind in enumerate(peak_inds_ttest_opt):
									...
									if strand != '+' and strand != '-':
										if strand_inferred == '+' or strand_inferred == '-':
											o.write('\t'.join([chrom, str(int(this_pos)), str(int(this_pos) + 1), name, fc, str(ksp),strand_inferred]) + '\n')
											#o.write('\t'.join([chrom, str(int(this_pos)), str(int(this_pos) + 1), name, fc,strand_inferred]) + '\n')
										else:
											o.write('\t'.join([chrom, str(int(this_pos)), str(int(this_pos) + 1), name, fc,str(ksp)]) + '\n')
											#o.write('\t'.join([chrom, str(int(this_pos)), str(int(this_pos) + 1), name, fc]) + '\n')
									else:
										o.write('\t'.join([chrom, str(int(this_pos)), str(int(this_pos) + 1), name, fc, str(ksp),strand]) + '\n')
