...
#my ($inp,$out,$ENS) = @ARGV;
my $inp = "SNU398/predicted.SANPolyA.txt";
my $out = "SNU398/new_pastype.predicted.SANPolyA.txt";
...
open FILE,"awk '(\$1 ~ \"+\")' $inp | sort -t ':' -k1,1 -k2,2n |";
while(<FILE>){
	chomp;
	next if $_ =~ /#/;
...
open FILE,"awk '(\$1 ~ \"-\")' $inp | sort -t ':' -k1,1 -k2,2nr |";
while(<FILE>){
	chomp;
	next if $_ =~ /#/;
	next if $_ =~ /predict_pasid/;
