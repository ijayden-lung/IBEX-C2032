...
#my ($inp,$out,$ENS) = @ARGV;
my $inp = "K562/predicted.txt";
my $out = "K562/new_pastype.predicted.txt";
...
my @INP = <FILE>;
open OUT,">$out";
print OUT "pas_id\tpas_type\tchr\tposition\tstrand\tsymbol\textend_length\tbewteen_gene\tbiotype\tgt_pas_id\tgt_diffscore\n";
foreach(@INP){
	chomp;
	next if $_ =~ /#/;
	next if $_ =~ /predict_pasid/;
	...
	print OUT "$pas_id\t$pas_type{$pas_id}\t$biotype\t$gt_pas\t$gt_diff\t$score\n";
}
