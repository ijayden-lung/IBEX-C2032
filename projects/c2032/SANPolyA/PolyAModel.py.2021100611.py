...
def LSTM(inputs):
	x = Conv1D(filters= 16, kernel_size= 10, padding = 'valid', kernel_regularizer = regularizers.l2(1e-4), bias_regularizer = regularizers.l2(1e-4))(inputs)
...
def PolyA_CNN(combination,length,seq_kernel_size=6,cov_kernel_size=6,pool_size=6):
	...
	if combination == 'SCP':
	...
	elif combination == 'UNET' :
		print ('input_layers = [seq_input,cov_input]')
		x = UNET(seq_input)
		y = UNET(cov_input)
		input_layers = [seq_input,cov_input]
		outLayer= Dense(1, activation='sigmoid',kernel_initializer=fc_initializer)(concatenate([x,y]))
	elif combination == 'DCNN' :
		print ('input_layers = [seq_input,cov_input]')
		x = DCNN(seq_input)
		y = DCNN(cov_input)
		input_layers = [seq_input,cov_input]
		...
		x = CNN(cov_input,cov_kernel_size,pool_size)
		input_layers = cov_input
		outLayer= Dense(1, kernel_regularizer = regularizers.l2(5e-4),bias_regularizer = regularizers.l2(5e-4),kernel_initializer=fc_initializer,activation='sigmoid')(x)
	elif combination == 'P':
