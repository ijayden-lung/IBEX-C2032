...
def write_file(dict_line,chromosome,strand,save_block,start,pre_pos,count,reference,window,name):
	...
	for i,pos in enumerate(pos_array):
	    ...
	    while(pos-pre_pos>=1):
	        if pre_pos not in dict_line.keys():
	            ...
	            #ww.write('%s:%d:%s\t%d\t%s\n'%(chromosome,pre_pos,strand,0,reference[chromosome][pre_pos-1]))
	        pre_pos += 1
...
def split(root_dir,block_length,input_file,reference,window,chromosome,strand,name):
	...
	for i, line in enumerate(lines):
	    ...
	    if(count>len_blocks and pos-pre_pos>1000 and save_block<num_blocks-1):
	        write_file(dict_line,chromosome,strand,save_block,start,pre_pos,count,reference,window)
	        count = 0
	        save_block += 1
	        start  = pos
	...
	write_file(dict_line,chromosome,strand,save_block,start,pre_pos,count,reference,window,name)
