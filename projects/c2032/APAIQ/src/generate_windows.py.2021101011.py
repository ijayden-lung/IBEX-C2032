...
def get_genome_sequence(fa_file,start,end):
    f = open(fa_file,"r")
    line = f.readline()
    line = line.rstrip('\n')
    f.close()
...
def split_chr_bedGraph2(root_dir,input_file,chromosome,strand,window,fa_file,depth,start,end):
    ...
    reference = get_genome_sequence('%s.%s.fa'%(fa_file,chromosome),start-window,end+window)
    with open(input_file, "r") as bg_file:
        lines = bg_file.readlines()
        for i,line in enumerate(lines):
            line = line.rstrip('\n')
            ...
            for pos in range(pos1-extend1,pos2+extend2):
                ...
                base = reference[pos-start+window]
                if(base == 'N'):
                    continue
                rpm = 0
