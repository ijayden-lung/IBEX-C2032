...
def normalize(data,pseudo_count,factor_dict):
...
def Evaluate(cov_data,pasid,out,window,model,pseudo_count,factor_dict):
    print('### Evaluation data processing')
    cov_data = normalize(cov_data,pseudo_count,factor_dict)
...
if __name__ == "__main__":
    model  = '../model/k562.ckpt'
    factor_path = '../model/normalize_factor'
    input_file = 'test21.txt'
   ...
   # bg_minus = "../../Split_BL6/STAR/rev1.bedGraph"
    ...
    Evaluate(data,pasid,out,window,model,pseudo_count,factor_dict)
