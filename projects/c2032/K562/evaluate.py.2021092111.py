...
def dataProcessing(data_path,RNASeqRCThreshold,polyA_file):
	data1 = []
	data2 = []
	data3 = []
	PASID = []
	...
	with open(data_path,'r') as f:
		for line in f:
			...
			if (line[0]!='#pas_id')&(line[0]!='pas_id'):
				...
				trimMean = trimmedMean(COVERAGE[0:100])
				chromosome=line[2]
				position=int(line[3])
				strand=line[4]
...
def main():
	...
	OUT=open(out,'w')
	seq_data,cov_data,polyA_data,pas_id= dataProcessing(data,RNASeqRCThreshold,polyA_file)
	keras_Model = PolyA_CNN(combination);
